    ...
    private static String getChar(int x, int y){
        ...
        for (Human human: main.allTeam) {
            if (human.getCoords()[0] == x && human.getCoords()[1] == y){
                ...
                if (main.darkTeam.contains(human)) out = "|" + (AnsiColors.ANSI_CYAN + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (main.holyTeam.contains(human)) out = "|" + (AnsiColors.ANSI_YELLOW + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
